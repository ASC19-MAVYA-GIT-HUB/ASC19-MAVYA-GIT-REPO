...
SELECT Name, ListPrice,
       ListPrice - AVG(ListPrice) OVER () AS PriceDifferenceFromAvg
FROM SalesLT.Product;
...
-- 89. What is the moving average of sales over the last 3 orders?
SELECT SalesOrderID, OrderDate, TotalDue,
       AVG(TotalDue) OVER (ORDER BY OrderDate ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) AS MovingAvgTotalDue
FROM SalesLT.SalesOrderHeader;
...
SELECT Name, ListPrice,
       CUME_DIST() OVER (ORDER BY ListPrice) AS PriceCumeDist
FROM SalesLT.Product;
...
SELECT soh.SalesOrderID,
       ...
       SUM(sod.OrderQty) AS TotalItems,
       soh.SubTotal,
       soh.TaxAmt,
       soh.Freight,
       soh.TotalDue,
       ROUND(((soh.TotalDue - soh.SubTotal) / soh.SubTotal) * 100, 2) AS AdditionalCostPercent
FROM SalesLT.SalesOrderHeader soh
JOIN SalesLT.Customer c ON soh.CustomerID = c.CustomerID
JOIN SalesLT.SalesOrderDetail sod ON soh.SalesOrderID = sod.SalesOrderID
GROUP BY soh.SalesOrderID, c.FirstName, c.LastName, soh.OrderDate, soh.ShipDate,
...
SELECT p1.Name AS ProductA, p2.Name AS ProductB, COUNT(*) AS TimesBoughtTogether
FROM SalesLT.SalesOrderDetail sod1
JOIN SalesLT.SalesOrderDetail sod2 
...
FROM (
    SELECT c.CustomerID,
           ...
           CASE 
               WHEN SUM(soh.TotalDue) >= 10000 THEN 'High Value'
...
SELECT p.Name,
       ...
       ROUND(COALESCE(SUM(sod.LineTotal), 0) / NULLIF(p.StandardCost, 0), 2) AS TurnoverRatio
FROM SalesLT.Product p
LEFT JOIN SalesLT.SalesOrderDetail sod ON p.ProductID = sod.ProductID
